...
inline void bind_x(const int * x)
{
#ifdef CUSP_USE_TEXTURE_MEMORY    
    size_t offset = size_t(-1);
    CUDA_SAFE_CALL(cudaBindTexture(&offset, tex_x_int, x));
    if (offset != 0)
...
inline void unbind_x(const int * x)
{
#ifdef CUSP_USE_TEXTURE_MEMORY
    CUDA_SAFE_CALL(cudaUnbindTexture(tex_x_int));
#else
    throw cusp::runtime_exception("texture support was not enabled");
#endif
}
...
__inline__ __device__ int fetch_x(const int& i, const int * x)
{
#ifdef CUSP_USE_TEXTURE_MEMORY
    if (UseCache)
		return tex1Dfetch(tex_x_int, i);
    else
#endif
        return x[i];
}
