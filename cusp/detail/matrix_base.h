...
namespace detail
{
    ...
    template<typename IndexType, typename MemorySpace, typename Format>
    class binary_matrix_base
    {
        public:
            typedef IndexType   index_type;
            typedef Format      format;
            ...
            template <typename Matrix>
            binary_matrix_base(const Matrix& m)
                : num_rows(m.num_rows), num_cols(m.num_cols), num_entries(m.num_entries) {}

            binary_matrix_base(size_t rows, size_t cols)
                : num_rows(rows), num_cols(cols), num_entries(0) {}

            binary_matrix_base(size_t rows, size_t cols, size_t entries)
                : num_rows(rows), num_cols(cols), num_entries(entries) {}
