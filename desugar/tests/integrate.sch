


; Home spun foldl which allows us to employ and test additional user-land higher-order functions
(define (myfold foldf acc lst)
        (if (null? lst)
            acc
            (myfold foldf (foldf (car lst) acc) (cdr lst))))


; A function for numerical integration over fun: Num -> Num between start and stop with delta precision/step using a quadrature rule
(define (integrate fun rule start stop delta) 
        (letrec ([generatepoints (lambda (pointslst)
                (let ([last (car (car pointslst))])
                     (if (< last start)
                         (cdr pointslst)
                         (generatepoints (cons (cons (- last delta) last) pointslst)))))])
        (myfold (lambda (pnts sum) (+ sum (rule fun pnts))) 0 (generatepoints (list (cons (- stop delta) stop))))))


; a function taking a function, pair of points, and a delta, which returns the area of that slice by the midpoint rule
(define (midpoint_rule func points)
        (* (- (cdr points) (car points))
           (func (/ (+ (car points) (cdr points)) 2.0))))


; a quadrature rule for calculating the area of a slice as a trapazoid
(define (trapezoidal_rule func points)
        (let ([a (func (car points))]
              [b (func (cdr points))])
             (let ([s (min a b)]
                   [b (max a b)])
                  (* (- (cdr points) (car points))
                     (+ s (/ (- b s) 2.0))))))


; a rule for overapproximating based on the largest of two points
(define (biggest_rule func points)
        (let ([a (func (car points))]
              [b (func (cdr points))])
             (let ([b (max a b)])
                  (* (- (cdr points) (car points)) b))))


; a rule for underapproximating based on the smallest of two points
(define (smallest_rule func points)
        (let ([a (func (car points))]
              [b (func (cdr points))])
             (let ([s (min a b)])
                  (* (- (cdr points) (car points)) s))))


; All rules
(define rules (list midpoint_rule trapezoidal_rule biggest_rule smallest_rule

		(lambda (w z) (midpoint_rule w z)) (lambda (w z) (trapezoidal_rule w z)) (lambda (w z) (biggest_rule w z)) (lambda (w z) (smallest_rule w z))
		(lambda (w z) (midpoint_rule w z)) (lambda (w z) (trapezoidal_rule w z)) (lambda (w z) (biggest_rule w z)) (lambda (w z) (smallest_rule w z))
		(lambda (w z) (midpoint_rule w z)) (lambda (w z) (trapezoidal_rule w z)) (lambda (w z) (biggest_rule w z)) (lambda (w z) (smallest_rule w z))
		(lambda (w z) (midpoint_rule w z)) (lambda (w z) (trapezoidal_rule w z)) (lambda (w z) (biggest_rule w z)) (lambda (w z) (smallest_rule w z))))


; Some functions to integrate over
(define funcs (list (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))(lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))(lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))(lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))(lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    
                    
                    
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))(lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))(lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))(lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))(lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    ))


(myfold (lambda (rule na) (myfold (lambda (afunc na) (pretty-print (integrate afunc rule 0.0 2.0 0.001))) na funcs)) (void) rules)



























; Home spun foldl which allows us to employ and test additional user-land higher-order functions
(define (myfold2 foldf acc lst)
        (if (null? lst)
            acc
            (myfold foldf (foldf (car lst) acc) (cdr lst))))


; A function for numerical integration over fun: Num -> Num between start and stop with delta precision/step using a quadrature rule
(define (integrate2 fun rule start stop delta) 
        (letrec ([generatepoints2 (lambda (pointslst)
                (let ([last (car (car pointslst))])
                     (if (< last start)
                         (cdr pointslst)
                         (generatepoints2 (cons (cons (- last delta) last) pointslst)))))])
        (myfold2 (lambda (pnts sum) (+ sum (rule fun pnts))) 0 (generatepoints2 (list (cons (- stop delta) stop))))))


; a function taking a function, pair of points, and a delta, which returns the area of that slice by the midpoint rule
(define (midpoint_rule2 func points)
        (* (- (cdr points) (car points))
           (func (/ (+ (car points) (cdr points)) 2.0))))


; a quadrature rule for calculating the area of a slice as a trapazoid
(define (trapezoidal_rule2 func points)
        (let ([a (func (car points))]
              [b (func (cdr points))])
             (let ([s (min a b)]
                   [b (max a b)])
                  (* (- (cdr points) (car points))
                     (+ s (/ (- b s) 2.0))))))


; a rule for overapproximating based on the largest of two points
(define (biggest_rule2 func points)
        (let ([a (func (car points))]
              [b (func (cdr points))])
             (let ([b (max a b)])
                  (* (- (cdr points) (car points)) b))))


; a rule for underapproximating based on the smallest of two points
(define (smallest_rule2 func points)
        (let ([a (func (car points))]
              [b (func (cdr points))])
             (let ([s (min a b)])
                  (* (- (cdr points) (car points)) s))))


; All rules
(define rules2 (list midpoint_rule trapezoidal_rule biggest_rule smallest_rule

		(lambda (w z) (midpoint_rule2 w z)) (lambda (w z) (trapezoidal_rule2 w z)) (lambda (w z) (biggest_rule2 w z)) (lambda (w z) (smallest_rule2 w z))
		(lambda (w z) (midpoint_rule2 w z)) (lambda (w z) (trapezoidal_rule2 w z)) (lambda (w z) (biggest_rule2 w z)) (lambda (w z) (smallest_rule2 w z))
		(lambda (w z) (midpoint_rule2 w z)) (lambda (w z) (trapezoidal_rule2 w z)) (lambda (w z) (biggest_rule2 w z)) (lambda (w z) (smallest_rule2 w z))
		(lambda (w z) (midpoint_rule2 w z)) (lambda (w z) (trapezoidal_rule2 w z)) (lambda (w z) (biggest_rule2 w z)) (lambda (w z) (smallest_rule2 w z))))


; Some functions to integrate over
(define funcs2 (list (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))(lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))(lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))(lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))(lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    
                    
                    
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))(lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))(lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))(lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))(lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    (lambda (x) (sqrt x))
                    (lambda (x) (* x x))
                    (lambda (x) (* x x x))
                    (lambda (x) (* x x 2))
                    (lambda (x) (* x x 3))
                    (lambda (x) (* x 2))
                    (lambda (x) (* x 3))
                    (lambda (x) (+ x 1))
                    (lambda (x) (+ x 2))
                    (lambda (x) (/ x 3.0))
                    (lambda (x) (expt x x))
                    (lambda (x) (- (expt x x) (* x x)))
                    (lambda (x) 2)
                    (lambda (x) (+ (* x x) x))
                    (lambda (x) (- (* x x) x))
                    (lambda (x) (/ x (* x x)))
                    (lambda (x) (max (* x x) (sqrt x)))
                    (lambda (x) (min (* x x) (sqrt x)))
                    ))


(myfold2 (lambda (rule na) (myfold2 (lambda (afunc na) (pretty-print (integrate2 afunc rule 0.0 2.0 0.001))) na funcs2)) (void) rules2)






